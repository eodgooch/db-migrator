...
RollbackService.prototype.rollback = async function rollback (
  currentPath,
  targetVersion
) {
  ...
  if (targetVersion == 'initial') {
    ...
    if (
      this._scriptService.versionExists(scriptVersions, targetVersion) == false
    ) {
      throw this._messages.INVALID_TARGET_VERSION + targetVersion
    }

    if (currentVersion < targetVersion) {
      throw 'Cannot rollback to newer version. Current: ' +
        currentVersion +
        ', target: ' +
        targetVersion +
        '.'
...
RollbackService.prototype._rollbackVersion = async function _rollbackVersion (
  versionInfo
) {
