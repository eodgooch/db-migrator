...
var persister = function(connection, tableName) {
    ...
    helper.createTable = function() {
        var sql = "CREATE TABLE " + tableName + " (id int, migrated_at timestamptz DEFAULT NOW())";
        console.log(sql);
    ...
    helper.getAll = Promise.coroutine(function* (sql) {
        var result = yield client.query("SELECT id, migrated_at FROM " + tableName + " ORDER BY id DESC;");

        return result.rows;
    })

    helper.addVersion = function(version) {
        return client.query("INSERT INTO " + tableName + " (id) VALUES ($1);", [version]);
    }
